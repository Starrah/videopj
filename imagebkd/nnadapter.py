    ...
    def run(self):
        ...
        for otp in self.otpList:
            if NNList[otp.type]["isMultiInput"]:
                ...
                interfaceObj = NNInterface(self.images, outputpath, otp, self.parallelCallback, self.lock, self.allowParallel)
                ...
                if le == 1:
                    ...
                    interfaceObj = NNInterface(image, outputpath, otp, self.parallelCallback, self.lock, self.allowParallel)
                    if self.allowParallel:
                        self.childThreads.append(interfaceObj)
                        interfaceObj.start()
                    else:
...
class NNInterface(threading.Thread):
    def __init__(self, inputPath, outputPath, otpObj, messageCallback=None, lock=None, needCopy=False):
        super().__init__()
    ...
    def NNCall(self):
        ...
        if self.needCopy:
            if isinstance(self.inputPath, list):
                aimPaths = []
                for one in self.inputPath:
                    aimPath = path.join(MEDIA_ROOT, "upload", str(self.otpObj.type), path.relpath(one, path.join(MEDIA_ROOT, "upload")))
                    dirname = path.dirname(aimPath)
                    if not path.exists(dirname):
                        os.makedirs(dirname)
                    with self.lock:
                        shutil.copyfile(one, aimPath)
                    aimPaths.append(aimPath)
                self.inputPath = aimPaths
            else:
                one = self.inputPath
                aimPath = path.join(MEDIA_ROOT, "upload", str(self.otpObj.type),
                                    path.relpath(one, path.join(MEDIA_ROOT, "upload")))
                dirname = path.dirname(aimPath)
                if not path.exists(dirname):
                    os.makedirs(dirname)
                with self.lock:
